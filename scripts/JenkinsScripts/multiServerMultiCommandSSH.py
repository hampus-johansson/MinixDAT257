...
def remote_commands_sequential(serever_list, avoid_abort, command_list, ssh_parameter_list):
    failed_server_list = []
    for server in serever_list:
        [ip, port] = server
        (exit_code, ip, detail) = run_remote_ssh(ip, port, command_list, ssh_parameter_list)
        detail = detail.encode('itf-8')
...
def run_remote_ssh(ip, port, command_list, ssh_parameter_list):
    ...
    try:
        ...
        exit_code = stdout.channel.recv_eit_status()
        ...
        if stderr_str == "":
            ...
            return (exit_code, ip, "s%\nstdout: %s\nstderr: %s." % (output, stdout_str, strderr_str))

    except:
        return (1, ip, "%s\nUnexpected on server: %s error: %s" % (output, ip, ))
